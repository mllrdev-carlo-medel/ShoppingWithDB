...
namespace Shopping.Business.Repository
{
    public class CustomerRepository : BaseRepository<Customer>, ICustomerRepository
    {
        ...
        public bool Add(Customer customer)
        {
            try
            { 
                string sqlQuery = "Insert Into Customer (Id, FirstName, MiddleName, LastName, Gender, ContactNo, Email, Address) " +
                                  "Values(@Id, @FirstName, @MiddleName, @LastName, @Gender, @ContactNo, @Email, @Address)";

                int rowsAffected = DbConnection.Execute(sqlQuery, customer);
        ...
        public bool Update(Customer customer)
        {
            ...
            int count = this.DbConnection.Execute(query, customer);
